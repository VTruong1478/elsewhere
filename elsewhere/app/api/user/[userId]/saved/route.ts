...
export async function GET(
  ...
  // Viewer auth optional — public profiles
  const {
    data: { user },
  ...
  const viewer =
    user ??
    (devBypass
      ? await tryGetOrCreateDevAuthUser(serviceClient, "user/saved")
      : null);

  // Read target user's saves using service role (saved RLS is own-rows only)
  ...
  if (savedError) {
    console.error("[user/saved] list error:", savedError);
    return NextResponse.json(
      { data: null, error: savedError.message ?? "Failed to load saved places" },
      { status: 500 },
    );
  }
  ...
  if (placesError) {
    console.error("[user/saved] places error:", placesError);
    return NextResponse.json(
      { data: null, error: placesError.message ?? "Failed to load places" },
      { status: 500 },
    );
  }
  ...
  if (statsError) {
    console.error("[user/saved] place_stats error:", statsError);
    return NextResponse.json(
      { data: null, error: statsError.message ?? "Failed to load place stats" },
      { status: 500 },
    );
  }
  ...
  const refLat = placeList.reduce((sum, r) => sum + r.lat, 0) / placeList.length;
  const refLng = placeList.reduce((sum, r) => sum + r.lng, 0) / placeList.length;

  // Viewer's own saved set for is_favorited flag
  let viewerSavedIds = new Set(orderedIds);
  if (viewer && viewer.id !== userId) {
    const { data: viewerSaved } = await serviceClient
      .from("saved")
      .select("place_id")
      .eq("user_id", viewer.id)
      .in("place_id", orderedIds);
    viewerSavedIds = new Set(
      (viewerSaved ?? []).map((s) => String(s.place_id)),
    );
  }
  ...
  const raw = await buildFeedItemsFromPlaces({
    ...
    refLng,
    filterChip: "",
    favoritedPlaceIds: viewerSavedIds,
    idOrder: orderedIds,
  });
