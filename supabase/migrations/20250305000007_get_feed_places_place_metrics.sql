-- Return place-level metrics (noise_level, tables_level, outlets_level) from places
-- so cards show them when set in Supabase, even with 0 ratings.
-- Drop first because return type (new columns) cannot be changed with CREATE OR REPLACE.
