-- Rebuild public.place_type so allowed labels are only cafe | library | bookstore | tea_shop.
-- Requires moving any rows still stored under the old label before the cast below.

UPDATE public.places
SET place_type = 'tea_shop'::public.place_type
WHERE place_type::text = 'boba_shop';

UPDATE public.place_submissions
SET place_type = 'tea_shop'
WHERE lower(trim(place_type)) = 'boba_shop';

DROP FUNCTION IF EXISTS public.get_feed_places(numeric, numeric, numeric, text, text);
DROP FUNCTION IF EXISTS public.get_feed_places(double precision, double precision, double precision, text, text);

CREATE TYPE public.place_type_new AS ENUM ('cafe', 'library', 'bookstore', 'tea_shop');

ALTER TABLE public.places
  ALTER COLUMN place_type TYPE public.place_type_new
  USING (place_type::text::public.place_type_new);

DROP TYPE public.place_type;

ALTER TYPE public.place_type_new RENAME TO place_type;
