-- Bookstores filter chip: restrict to place_type = 'bookstore' (same pattern as cafes/libraries).
...
AS $$
  ...
  WHERE
    ...
    AND (
      ...
      OR (filter_chip = 'bookstores' AND lower(trim(p.place_type)) = 'bookstore')
      OR (filter_chip NOT IN ('cafes', 'libraries', 'bookstores'))
    );
$$;
