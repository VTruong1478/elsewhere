-- Tea shop filter chip: place_type = 'tea_shop' (filter param: tea_shops).
-- RETURNS place_type must match public.places.place_type (enum), not text, or
-- CREATE OR REPLACE fails with: cannot change return type of existing function.
