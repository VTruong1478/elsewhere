-- get_feed_places was recreated without SECURITY DEFINER (see 20260422120000 /
-- 20260423120000). As SECURITY INVOKER it runs with the caller's role; combined
-- with tightened grants on public.ratings, the feed RPC could fail with
-- "permission denied for table ratings" even though the function body only
-- reads places + place_stats. Restore definer execution + pinned search_path.
